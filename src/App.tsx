...
import { wwData, WWKey, WW } from './jobs/windwalker';
...
export default function App() {
  ...
  const [cds, setCds] = useState(() => Object.fromEntries(
    Object.keys(WW).map(k => [k, 0])
  ) as Record<WWKey, number>);
  ...
  const click = (key: WWKey) => {
    ...
    setItems(it => {
      const start = Math.max(time, cds[key]);
      const end = start + (spell.castEff ?? spell.gcd);
      setCds(c => ({ ...c, [key]: start + (spell.cooldown ?? 0) }));
      setTime(end);
      return [
        ...it,
        {
