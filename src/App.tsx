...
export default function App() {
  ...
  const click = (key: WWKey) => {
    ...
    if (key === 'AA') {
      ...
      extraBuffs.push({ id: nextBuffId - 1, key: 'BLG', start: now, end: now + 6, label: 'Blessing', src: id, group: 4, multiplier: 1.15 } as any);
      ...
      extraBuffs.push({ id: nextBuffId - 1, key: 'BLG', start: now + castDur, end: now + castDur + 8, label: 'Blessing', src: id, group: 4, multiplier: 1.15 } as any);
      setNextBuffId(nextBuffId - 2);
    } else if (key === 'CC') {
      ...
      extraBuffs.push({ id: nextBuffId - 1, key: 'BLG', start, end: start + 6, label: 'Blessing', src: id, group: 4, multiplier: 1.15 } as any);
      setNextBuffId(nextBuffId - 2);
    } else if (key === 'BL') {
  ...
  const buffItems: TLItem[] = [
    ...
    ...otherBuffs.map(b => ({
      ...
      label: b.label,
      className: b.key === 'BLG' ? 'blessing' : 'buff',
    })),
  ];
