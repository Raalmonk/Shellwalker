...
describe('cooldown integration', () => {
  it('AA cooldown ends at 23.33 s with dragon', () => {
    const buffs: Buff[] = [b('AA_BD', 0, 6)];
    const aa: SkillCast = { id: 'AA', start: 0, base: 30 };
    expect(getEndAt(aa, buffs)).toBeCloseTo(23.33, 2);
  });

  it('FoF ends at 17.33 s after AA', () => {
    const buffs: Buff[] = [b('AA_BD', 0, 6)];
    const fof: SkillCast = { id: 'FoF', start: 0, base: 24 };
    expect(getEndAt(fof, buffs)).toBeCloseTo(17.33, 2);
  });
